...
from scripts.models import Vulnerability, EPSSScore
...
class EPSSFilterAgent(BaseAgent):
    ...
    def _extract_epss_score(self, vuln: Dict[str, Any]) -> float:
        ...
        if 'epssScore' in vuln and isinstance(vuln['epssScore'], dict):
            if 'score' in vuln['epssScore']:
                try:
                    return float(vuln['epssScore']['score'])
                except (ValueError, TypeError):
                    pass
                    
        if 'epss' in vuln and isinstance(vuln['epss'], dict):
            if 'score' in vuln['epss']:
                try:
