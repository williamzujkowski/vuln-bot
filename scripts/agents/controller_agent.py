...
class ControllerAgent(BaseAgent):
    ...
    def __init__(self, cache_dir: Path = None):
        ...
        self.pipeline_config = {
            "agents": ["cve_fetch", "static_page", "dashboard", "ci"],
            "max_concurrent": 2,
            "retry_attempts": 3,
        ...
        try:
            ...
            for agent_name in self.pipeline_config["agents"]:
                ...
                try:
                    ...
                    agent_duration = (
                        datetime.now(timezone.utc) - agent_start
                    ).total_seconds()
            ...
            results["metrics"] = {
                ...
                "agents_executed": len(
                    [a for a in results["agents"].values() if "duration_seconds" in a]
                ),
                "agents_succeeded": len(
                    [a for a in results["agents"].values() if a.get("success")]
                ),
                "agents_failed": len(
                    [a for a in results["agents"].values() if not a.get("success")]
                ),
            }
    ...
    def get_dependencies(self) -> Set[str]:
        ...
        deps = {"scripts/agents/", "scripts/main.py", ".eleventy.js", "package.json"}
    ...
    async def get_pipeline_status(self) -> Dict[str, Any]:
        ...
        status = {
            ...
            "pipeline": {
                ...
                "missing_agents": [
                    name
                    for name in self.pipeline_config["agents"]
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        for name, agent in self.agents.items():
            try:
                ...
                is_healthy = (
                    not agent.is_running  # Not stuck
                    or len(agent.errors) < 5  # Not too many errors
                    or (
                        agent.last_run
                        and (
                            datetime.now(timezone.utc) - agent.last_run
                        ).total_seconds()
                        < 86400
                    )  # Ran recently
                )

                health["agents"][name] = {"healthy": is_healthy, "status": agent_status}
