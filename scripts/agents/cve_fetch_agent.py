...
class CVEFetchAgent(BaseAgent):
        ...
        try:
            ...
            if config.get("adaptive"):
                ...
                if recent_metrics:
                    ...
                    avg_cves_per_day = sum(
                        m["vulnerability_count"] / m["days_covered"]
                        for m in recent_metrics
                    ) / len(recent_metrics)

                    # If rate is high, reduce days back to stay under max_cves_per_run
                    if (
                        avg_cves_per_day * config["days_back"]
                        > config["max_cves_per_run"]
                    ):
                        optimal_days = int(
                            config["max_cves_per_run"] / avg_cves_per_day
                        )
    ...
    async def get_fetch_stats(self) -> Dict[str, Any]:
        ...
        harvest_history = await asyncio.to_thread(
            self.cache_manager.get_harvest_history, 10
        )
