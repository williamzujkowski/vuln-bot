...
class AgentManager:
        ...
        try:
            ...
            if results['success']:
                self.logger.info(
                    ...
                    duration=results.get('metrics', {}).get('total_duration_seconds', 0)
                )
            else:
                self.logger.error(
    ...
    async def run_agent(self, agent_name: str, force: bool = False, **kwargs) -> Dict[str, any]:
        ...
        try:
            ...
            self.logger.error("Agent execution failed", agent_name=agent_name, error=str(e))
            raise
