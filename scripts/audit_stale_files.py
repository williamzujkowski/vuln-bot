...
@click.command()
@click.option(
    "--output-dir", type=Path, default=Path("public"), help="Output directory to audit"
)
@click.option(
    "--api-dir", type=Path, default=Path("api"), help="API directory with valid data"
)
...
def audit_stale_files(output_dir: Path, api_dir: Path, min_epss: float):
    ...
    if output_dir.exists():
        ...
        for json_file in output_dir.glob("api/cves/CVE-*.json"):
            ...
            if cve_id not in valid_cve_ids:
                stale_files.append(
                    {"path": str(json_file), "cve_id": cve_id, "type": "api_json"}
                )
        ...
        for html_file in output_dir.glob("CVE-*.html"):
            ...
            if cve_id not in valid_cve_ids:
                stale_files.append(
                    {"path": str(html_file), "cve_id": cve_id, "type": "html_page"}
                )
...
"""
    ...
    if stale_files:
        ...
        summary += "\n## Action Required\n"
        summary += "Run `npm run clean` or `python -m scripts.cleanup_stale_files --force-purge` to remove these files.\n"
    else:
