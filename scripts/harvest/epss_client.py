...
class EPSSClient(BaseAPIClient):
    ...
    def fetch_epss_scores(
        ...
        try:
            ...
            score_date = date_parser.parse(response.get("score_date", datetime.utcnow().isoformat()))
            ...
            for item in data:
                ...
                try:
                    epss_score = EPSSScore(
                        ...
                        percentile=float(item.get("percentile", 0.0)) * 100,  # Convert to percentage
                        date=score_date,
                    )
    ...
    def fetch_daily_epss_file(self, date: Optional[datetime] = None) -> Dict[str, EPSSScore]:
