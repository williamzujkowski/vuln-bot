...
class VulnerabilityNormalizer:
    ...
    TAG_PATTERNS = {
        "authentication": re.compile(
            r"\b(auth|authentication|login|credential)\b", re.I
        ),
    ...
    def detect_exploitation_status(self, text: str) -> ExploitationStatus:
        ...
        status_mapping = {
            "active": ExploitationStatus.ACTIVE,
            "weaponized": ExploitationStatus.WEAPONIZED,
            "poc": ExploitationStatus.POC,
        }

        for status, keywords in self.EXPLOITATION_KEYWORDS.items():
            if any(keyword in text_lower for keyword in keywords):
                return status_mapping.get(status, ExploitationStatus.UNKNOWN)
        ...
        for vuln in vulnerabilities[1:]:
            ...
            for metric in vuln.cvss_metrics:
                if not any(
                    ...
                    and m.vector_string == metric.vector_string
            ...
            merged.affected_vendors = sorted(
                set(merged.affected_vendors + vuln.affected_vendors)
            )
            merged.affected_products = sorted(
                set(merged.affected_products + vuln.affected_products)
            )
            ...
            # Merge tags
            merged.tags = sorted(set(merged.tags + vuln.tags))
            ...
            if (
                vuln.attack_vector
                and vuln.attack_vector != "Unknown"
                and merged.attack_vector == "Unknown"
            ):
                merged.attack_vector = vuln.attack_vector
            if (
                vuln.attack_complexity
                and vuln.attack_complexity != "Unknown"
                and merged.attack_complexity == "Unknown"
            ):
                merged.attack_complexity = vuln.attack_complexity
            if (
                vuln.privileges_required
                and vuln.privileges_required != "Unknown"
                and merged.privileges_required == "Unknown"
            ):
                merged.privileges_required = vuln.privileges_required
            if (
                vuln.user_interaction
                and vuln.user_interaction != "Unknown"
                and merged.user_interaction == "Unknown"
            ):
                merged.user_interaction = vuln.user_interaction

        return merged

    def normalize_github_advisory(
