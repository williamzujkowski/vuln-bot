...
class BriefingGenerator:
    ...
    def generate_briefing_post(
        ...
        briefing_data = {
            ...
            "vulnerabilities": [],
            "risk_distribution": {
                "critical": len([v for v in batch.vulnerabilities if v.risk_score >= 90]),
                "high": len([v for v in batch.vulnerabilities if 70 <= v.risk_score < 90]),
                "medium": len([v for v in batch.vulnerabilities if 40 <= v.risk_score < 70]),
                "low": len([v for v in batch.vulnerabilities if v.risk_score < 40]),
            },
            "severity_distribution": {
                "CRITICAL": len([v for v in batch.vulnerabilities if v.severity.value == "CRITICAL"]),
                "HIGH": len([v for v in batch.vulnerabilities if v.severity.value == "HIGH"]),
                "MEDIUM": len([v for v in batch.vulnerabilities if v.severity.value == "MEDIUM"]),
                "LOW": len([v for v in batch.vulnerabilities if v.severity.value == "LOW"]),
            },
        }
        ...
        for vuln in top_vulns:
            ...
            vuln_data = {
                ...
                "references": [ref.url for ref in vuln.references[:3]],  # Top 3 references
            }
    ...
    def _generate_markdown_briefing(self, data: Dict[str, Any]) -> str:
        ...
        lines.extend([
            ...
            f"layout: post",
            f"tags: [vulnerability, briefing, security]",
        ...
        for i, vuln in enumerate(data["vulnerabilities"], 1):
            ...
            if vuln["tags"]:
                lines.append(f"**Tags**: {', '.join(f'`{tag}`' for tag in vuln['tags'])}")
                lines.append("")
        ...
        for source in data["sources"]:
            ...
            lines.append(f"- {status} {source['name']}: {source['count']} vulnerabilities")
