...
class HTMXDashboardGenerator:
    ...
    def generate_table_fragment(self, vulns: List[sqlite3.Row], page: int = 1, 
                              per_page: int = 50, sort_field: str = "epss_percentile",
        ...
        if sort_field == "risk_score":
            vulns = sorted(vulns, key=lambda v: self.calculate_risk_score(v), 
                         reverse=(sort_order == "desc"))
        else:
            vulns = sorted(vulns, key=lambda v: v[sort_field] or 0, 
                         reverse=(sort_order == "desc"))
    ...
    def filter_vulnerabilities(self, filter_type: str, value: str = "") -> List[sqlite3.Row]:
        ...
        if filter_type == "all":
            ...
            return [v for v in self.vulnerabilities 
                   if v['published_date'].startswith(today)]
        elif filter_type == "kev":
            return [v for v in self.vulnerabilities 
                   if v['tags'] and 'KEV' in json.loads(v['tags'])]
        elif filter_type == "network":
            return [v for v in self.vulnerabilities 
                   if v['attack_vector'] == 'NETWORK' or 
                      'network' in v['title'].lower() or 
                      'remote' in v['title'].lower()]
        else:
            return self.vulnerabilities
    ...
    def generate_charts_fragment(self) -> str:
        ...
        for range_def in epss_ranges:
            count = len([v for v in self.vulnerabilities 
    ...
    def generate_all_fragments(self):
        ...
        sort_fields = ["cve_id", "severity", "cvss_score", "epss_percentile", 
                      "risk_score", "published_date"]
        for field in sort_fields:
            # Generate both asc and desc versions
            for order in ["asc", "desc"]:
                with open(FRAGMENTS_DIR / "sort" / f"{field}_{order}.html", "w") as f:
                    f.write(self.generate_table_fragment(
