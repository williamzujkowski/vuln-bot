...
def reprocess_cve_file(file_path: Path, extractor: VendorProductExtractor):
    ...
    for cve in cves:
        ...
        title = cve.get('title', cve.get('cveId', ''))
        ...
        problematic = {'the', 'n/a', 'na', 'web', 'file', 'server', 'database',
                      'framework', 'application', 'system', 'none', 'unknown'}

        new_vendors = [v for v in old_vendors if v.lower() not in problematic and len(v) > 1]
        new_products = [p for p in old_products if p.lower() not in problematic and len(p) > 1]
...
def main():
    ...
    for json_file in api_dir.glob('*.json'):
        ...
        updated = reprocess_cve_file(json_file, extractor)
