...
async def test_live_site_data_quality():
    ...
    async with async_playwright() as p:
        ...
        try:
            print(
                "🌐 Testing live website: https://williamzujkowski.github.io/vuln-bot/"
            )
            await page.goto(
            ...
            for i, vuln in enumerate(vuln_data):
                ...
                if not vuln["cve_id"] or not re.match(
                    r"^CVE-\d{4}-\d+$", vuln["cve_id"]
                ):
                ...
                if vuln["products"]:
                    ...
                    if "/" in str(vuln["products"]):
                        row_issues.append(
                            f"Product field still contains vendor/product: {vuln['products']}"
                        )
                else:
                ...
                if vuln["published_date"]:
                    try:
                        ...
                        row_issues.append(
                            f"Invalid date format: {vuln['published_date']}"
                        )
                else:
                ...
                if vuln["exploitation_status"] not in valid_statuses:
                    row_issues.append(
                        f"Invalid exploitation status: {vuln['exploitation_status']}"
                    )

                # 10. Check for data consistency
                if (
                    vuln["products_list"]
                    and vuln["vendors_list"]
                    and len(vuln["products_list"]) > 0
                    and len(vuln["vendors_list"]) == 0
                ):
                ...
                if row_issues:
                    issues.append(
                        {"cve_id": vuln["cve_id"], "row": i + 1, "issues": row_issues}
                    )
